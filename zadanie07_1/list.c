...
struct element * list_join(struct element * first_list, struct element * second_list) {
  struct element * copy_first_list;
  struct element * copy_second_list;

  copy_first_list = list_copy(first_list);
  copy_second_list = list_copy(second_list);

  if (is_empty(first_list)) {
    return copy_second_list;
  }

  if (is_empty(second_list)) {
    return copy_first_list;
  }

  struct element * new_first_element = copy_first_list->pointer;

  copy_first_list->pointer = copy_second_list->pointer;
  copy_second_list->pointer = new_first_element;
...
// obsluga bledow: list_shift, list_fetch_first,
